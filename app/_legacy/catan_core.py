...
# === SPRINT_DEV_CARDS_MONKEYPATCH ===
# Added: Dev cards core methods for UI (buy_dev / play_dev). Minimal rules, expandable later.

import random

_S5_RES = ("wood","brick","sheep","wheat","ore")

def _s5_get_player_obj(self, pid: int):
    return self.players[pid]

def _s5_get_player_res(self, pid: int) -> dict:
    fn = getattr(self, "_get_player_res_dict", None)
    if callable(fn):
        return fn(pid)
    p = _s5_get_player_obj(self, pid)
    if hasattr(p, "res"):
        return p.res
    if isinstance(p, dict) and "res" in p:
        return p["res"]
...
def _s5_get_player_vp_ref(p):
    # returns (getter, setter)
    if hasattr(p, "vp"):
        return (lambda: int(getattr(p, "vp", 0))), (lambda v: setattr(p, "vp", int(v)))
    if isinstance(p, dict):
        return (lambda: int(p.get("vp", 0))), (lambda v: p.__setitem__("vp", int(v)))
    return (lambda: 0), (lambda v: None)

def _s5_get_player_dev_list(p):
    # normalize to list of dicts: [{"type": "...", "new": True/False}]
    if hasattr(p, "dev_cards"):
        if p.dev_cards is None:
            p.dev_cards = []
        return p.dev_cards
    if hasattr(p, "dev"):
        if p.dev is None:
    ...
    setattr(p, "dev_cards", [])
    return getattr(p, "dev_cards")

def _s5_ensure_bank(self):
    # reuse existing if present
    if not hasattr(self, "bank") or self.bank is None:
        self.bank = {r: 19 for r in _S5_RES}
    else:
        for r in _S5_RES:
            self.bank.setdefault(r, 19)

def _s5_ensure_dev_deck(self):
    # Base game dev deck: 25 cards
    # 14 Knight, 5 Victory Point, 2 Road Building, 2 Year of Plenty, 2 Monopoly
    if not hasattr(self, "dev_deck") or not self.dev_deck:
        deck = (["knight"] * 14) + (["victory_point"] * 5) + (["road_building"] * 2) + (["year_of_plenty"] * 2) + (["monopoly"] * 2)
        random.shuffle(deck)
        self.dev_deck = deck

def _s5_buy_dev(self, pid: int):
    # Cost: sheep + wheat + ore
    pres = _s5_get_player_res(self, pid)
    cost = {"sheep": 1, "wheat": 1, "ore": 1}
    for r, q in cost.items():
        if pres.get(r, 0) < q:
            raise ValueError("Not enough resources to buy dev card (need sheep+wheat+ore).")

    _s5_ensure_bank(self)
    _s5_ensure_dev_deck(self)

    # pay to bank
    for r, q in cost.items():
        pres[r] -= q
        self.bank[r] = self.bank.get(r, 0) + q

    card = self.dev_deck.pop()
    p = _s5_get_player_obj(self, pid)
    devlist = _s5_get_player_dev_list(p)
    devlist.append({"type": card, "new": True})

    # VP card: apply immediately
    if card == "victory_point":
        get_vp, set_vp = _s5_get_player_vp_ref(p)
        set_vp(get_vp() + 1)

    return card

def _s5_play_dev(self, pid: int, card_type: str, **kwargs):
    p = _s5_get_player_obj(self, pid)
    devlist = _s5_get_player_dev_list(p)

    idx = None
    for i, c in enumerate(list(devlist)):
        t = c.get("type") if isinstance(c, dict) else str(c)
        is_new = c.get("new", False) if isinstance(c, dict) else False
        if t == card_type and not is_new:
            idx = i
            break

    # allow playing immediately if UI doesn't track "new" yet (fallback)
    if idx is None:
        for i, c in enumerate(list(devlist)):
            t = c.get("type") if isinstance(c, dict) else str(c)
            if t == card_type:
    ...
    if idx is None:
        raise ValueError(f"No dev card '{card_type}' available.")

    card = devlist.pop(idx)
    _s5_ensure_bank(self)

    if card_type == "year_of_plenty":
        a = kwargs.get("a", "wood")
        b = kwargs.get("b", "brick")
        qa = int(kwargs.get("qa", 1))
        qb = int(kwargs.get("qb", 1))
        pres = _s5_get_player_res(self, pid)
        for r, q in ((a, qa), (b, qb)):
            if r not in _S5_RES: raise ValueError("Unknown resource")
            if self.bank.get(r, 0) < q: raise ValueError("Bank doesn't have enough")
            self.bank[r] -= q
            pres[r] = pres.get(r, 0) + q
        return {"played": "year_of_plenty", "a": a, "b": b, "qa": qa, "qb": qb}

    if card_type == "monopoly":
        r = kwargs.get("res", "wood")
        if r not in _S5_RES: raise ValueError("Unknown resource")
        pres = _s5_get_player_res(self, pid)
        taken = 0
        for opid in range(len(self.players)):
            if opid == pid: continue
            ores = _s5_get_player_res(self, opid)
            q = int(ores.get(r, 0))
            if q > 0:
                ores[r] -= q
                taken += q
        pres[r] = pres.get(r, 0) + taken
        return {"played": "monopoly", "res": r, "taken": taken}

    if card_type == "road_building":
        # placeholder: weвЂ™ll wire "2 free roads" into build rules next sprint
        # keep card consumed so UI behaves.
        return {"played": "road_building", "note": "Wiring free-roads into build rules next sprint."}

    if card_type == "knight":
        # placeholder: robber + largest army later
        return {"played": "knight", "note": "Robber/largest-army will be added later."}

    if card_type == "victory_point":
        # already applied on buy, nothing to do
        return {"played": "victory_point", "note": "VP applied on buy."}
...
# === PORT+BANK TRADE FIX (uses your actual core structures) ===
...
def _norm_kind(kind):
    ...
    s = str(kind).strip().lower()
    # generic 3:1
    if "3:1" in s or "3/1" in s or "3 to 1" in s or "3to1" in s or "generic" in s:
        return "3:1"
    # 2:1 resource ports (accept formats like "2:1 wheat", "wheat", etc.)
    for r in _RES:
        if r in s:
            return r
    return s

def _get_port_fields(port):
    # supports dict or object with attrs
    kind = getattr(port, "kind", None) if not isinstance(port, dict) else port.get("kind")
    a = (getattr(port, "a", None) if not isinstance(port, dict) else port.get("a"))
    b = (getattr(port, "b", None) if not isinstance(port, dict) else port.get("b"))
    # also accept v1/v2 naming
    if a is None:
        a = (getattr(port, "v1", None) if not isinstance(port, dict) else port.get("v1"))
    if b is None:
        b = (getattr(port, "v2", None) if not isinstance(port, dict) else port.get("v2"))
    return _norm_kind(kind), a, b

def _player_ports(self, pid: int) -> set[str]:
    ports = set()
    plist = getattr(self, "ports", None)
    if not plist:
        return ports
    ...
    for port in plist:
        kind, a, b = _get_port_fields(port)
        if kind is None:
            continue
...
def _trade_with_bank(self, pid: int, give_res: str, get_res: str, get_qty: int = 1) -> int:
    if self.phase != "main":
        raise ValueError("Trade only in main phase.")
    if give_res == get_res:
        raise ValueError("Pick different resources.")
    if give_res not in _RES or get_res not in _RES:
        raise ValueError("Unknown resource.")
    if get_qty < 1:
        raise ValueError("get_qty must be >= 1")
    ...
    p = self.players[pid]
    if p.resources.get(give_res, 0) < give_qty:
        raise ValueError(f"Not enough {give_res} (need {give_qty}).")
    if self.bank.stock.get(get_res, 0) < get_qty:
        raise ValueError("Bank is out of that resource.")

    # pay to bank
    p.resources[give_res] -= give_qty
    self.bank.put(give_res, give_qty)

    # receive from bank
    got = self.bank.take(get_res, get_qty)
    p.resources[get_res] += got

    return rate

# attach (override any broken sprint monkeypatches)
Game.player_ports = _player_ports
Game.best_trade_rate = _best_trade_rate
Game.trade_with_bank = _trade_with_bank

# === PORTS_CORE_FIX_V1 ===
# Ports-aware trade support (2:1 / 3:1 / 4:1). Works with both Bank object or dict bank.

def _pcf_norm_kind(kind):
    if kind is None:
        return None
    s = str(kind).strip().lower()
    ...
    for r in ("wood","brick","sheep","wheat","ore"):
        if r in s:
            return r
    if s in ("wood","brick","sheep","wheat","ore"):
        return s
    return s
...
def _pcf_get_res_dict(game, pid: int):
    p = game.players[pid]
    ...
    if hasattr(p, "res"):
        return p.res
    if isinstance(p, dict):
        if "resources" in p: return p["resources"]
        if "res" in p: return p["res"]
    raise RuntimeError("Cannot locate player resources dict (expected player.resources or player.res).")

def _pcf_owned_vertices(game, pid: int):
    p = game.players[pid]
    # this project core uses per-player sets
    if hasattr(p, "settlements") or hasattr(p, "cities"):
        s = set(getattr(p, "settlements", set()) or set())
        c = set(getattr(p, "cities", set()) or set())
        return s | c
    # fallback for other cores
    owned = set()
    if hasattr(game, "buildings"):
        for vid, data in getattr(game, "buildings", {}).items():
            try:
                owner, bkind = data
            except Exception:
                continue
            if owner == pid and bkind in ("settlement","city"):
                owned.add(_pcf_norm_vid(vid))
    if hasattr(game, "vertex_owner"):
        for vid, owner in getattr(game, "vertex_owner", {}).items():
            if owner == pid:
                owned.add(_pcf_norm_vid(vid))
    return owned

def _pcf_player_ports(game, pid: int) -> set[str]:
    ports = set()
    plist = getattr(game, "ports", None) or []
    if not plist:
        return ports
    owned = _pcf_owned_vertices(game, pid)
    for port in plist:
        if isinstance(port, dict):
            ...
            a = getattr(port, "a", None) or getattr(port, "v1", None)
            b = getattr(port, "b", None) or getattr(port, "v2", None)
        a = _pcf_norm_vid(a); b = _pcf_norm_vid(b)
        if a in owned or b in owned:
            k = _pcf_norm_kind(kind)
            if k:
                ports.add(k)
    return ports

def _pcf_best_trade_rate(game, pid: int, give_res: str) -> int:
    ports = _pcf_player_ports(game, pid)
    rate = 4
    if "3:1" in ports:
        rate = 3
    if give_res in ports:
...
def _pcf_bank_can_pay(bank, res: str, n: int) -> bool:
    if hasattr(bank, "can_pay"):
        return bool(bank.can_pay(res, n))
    stock = getattr(bank, "stock", None)
    if isinstance(stock, dict):
        return int(stock.get(res, 0)) >= n
    if isinstance(bank, dict):
        return int(bank.get(res, 0)) >= n
    return False

def _pcf_bank_take(bank, res: str, n: int) -> int:
    if hasattr(bank, "take"):
        return int(bank.take(res, n))
    stock = getattr(bank, "stock", None)
    if isinstance(stock, dict):
        ...
        return k
    if isinstance(bank, dict):
        have = int(bank.get(res, 0))
        k = min(have, n)
        bank[res] = have - k
        return k
    return 0

def _pcf_bank_put(bank, res: str, n: int) -> None:
    if hasattr(bank, "put"):
        bank.put(res, n); return
    stock = getattr(bank, "stock", None)
    if isinstance(stock, dict):
        stock[res] = int(stock.get(res, 0)) + int(n); return
    if isinstance(bank, dict):
        bank[res] = int(bank.get(res, 0)) + int(n); return

def _pcf_trade_with_bank(game, pid: int, give_res: str, get_res: str, get_qty: int = 1) -> int:
    if give_res == get_res:
        raise ValueError("Give and Get must be different.")
    if get_qty < 1:
        raise ValueError("get_qty must be >= 1")

    rate = _pcf_best_trade_rate(game, pid, give_res)
    give_qty = rate * int(get_qty)

    pres = _pcf_get_res_dict(game, pid)
    bank = getattr(game, "bank", None)
    if bank is None:
        raise RuntimeError("No bank on game")

    if int(pres.get(give_res, 0)) < give_qty:
        raise ValueError("Not enough resources to trade.")
    if not _pcf_bank_can_pay(bank, get_res, int(get_qty)):
        raise ValueError("Bank does not have enough of requested resource.")

    pres[give_res] = int(pres.get(give_res, 0)) - give_qty
    _pcf_bank_put(bank, give_res, give_qty)

    got = _pcf_bank_take(bank, get_res, int(get_qty))
    pres[get_res] = int(pres.get(get_res, 0)) + int(got)
    return rate

# Attach / override on Game
try:
    Game
except NameError:
    Game = None
...
if Game is not None:
    Game.player_ports = _pcf_player_ports
    Game.best_trade_rate = _pcf_best_trade_rate
    Game.trade_with_bank = _pcf_trade_with_bank