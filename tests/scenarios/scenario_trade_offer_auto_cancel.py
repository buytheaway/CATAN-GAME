...
def run(driver: GameDriver) -> Dict[str, Any]:
    ...
    res = driver.do({"type": "end_turn", "pid": 0})
    if not res.get("ok"):
    ...
    for o in g.trade_offers:
        if o.offer_id == offer_id:
            offer = o
            break
    if offer is None or offer.status != "canceled":
        driver.fail("offer not auto-canceled", kind="assertion", details={"status": getattr(offer, 'status', None)})
